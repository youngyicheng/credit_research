...
class CDSImpliedVolatilitySolver:
    ...
    def calculate_survival_probability(
        ...
        assert (
            survival_prob > 0 and survival_prob < 1
        ), "survival_prod should be between 0 and 1"
    ...
    def _calculate_G_function_approximate(
        ...
        G_approx = d ** (z + 0.5) * norm.cdf(
            -np.log(d) / (sigma * np.sqrt(u)) - z * sigma * np.sqrt(u)
        ) + d ** (-1 * z + 0.5) * norm.cdf(
            -np.log(d) / (sigma * np.sqrt(u)) + z * sigma * np.sqrt(u)
        )
    ...
    def calculate_intermediate_terms(
        self, S: float, D: float, sigma: float
    ) -> CGMIntermediateTerms:
        """
        计算中间变量，便于其他函数调用
    ...
    def calculate_cds_spread_continuous(
        self, S: float, D: float, sigma: float, R: float = 0.4
    ) -> float:
        """
    ...
    def calculate_cds_upfront(
        ...
        second_term = (
            -1 * cds_coupon / self.r * (P_0 - P_t * np.exp(-1 * self.r * self.t))
        )
    ...
    def objective_function(
        ...
        try:
            ...
            }, "Quote spread type not implemented yet"
            model_quote = (
    ...
    def solve_implied_volatility_brent(
        self,
        S: float,
        D: float,
        target_cds_quote: float,
    ...
    def solve_implied_volatility_minimize(
        self,
        S: float,
        D: float,
        ...
        try:
            result = minimize_scalar(
                obj_func_abs, bounds=(vol_min, vol_max), method="bounded"
            )
    ...
    def solve_implied_volatility(
        ...
        **kwargs :
        ...
        if self.cds_quote_type == CDSQuoteType.UPFRONT:
            assert (
                abs(target_cds_quote) <= 1.0
            ), "CDS upfront should be in raw form (e.g., 0.01 for 1%)"
    ...
    def delta_calculation(
        ...
        Returns:
        --------
        float : delta value (first derivative)
        """
    ...
    def gamma_calculation(
        ...
        Returns:
        --------
        float : gamma value (second derivative)
        """
        # Default step size if not provided
        if h is None:
            h = max(1e-4 * S, 1.0)
...
def demo_implied_volatility():
    ...
    for market_spread_bp in test_cases:
        ...
        if implied_vol_brent is not None:
            # 验证结果
            verification_spread = solver.calculate_cds_spread_continuous(
                S, D, implied_vol_brent, R
            )
            logger.info(
                f"Brent方法结果验证: 模型价差 = {verification_spread*10000:.2f}bp"
            )

        if implied_vol_minimize is not None:
            verification_spread = solver.calculate_cds_spread_continuous(
                S, D, implied_vol_minimize, R
            )
            logger.info(
                f"最小化方法结果验证: 模型价差 = {verification_spread*10000:.2f}bp"
            )


# test program to verify consistency with cgm_core.py(unimportant here)
def test_consistency_with_cgm_core():
    ...
    logger.info(
        f"使用波动率 {test_sigma:.4f} 计算的CDS价差: {cds_spread:.6f} ({cds_spread*10000:.2f}bp)"
    )
